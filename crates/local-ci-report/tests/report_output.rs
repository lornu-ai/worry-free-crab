...
fn test_json_schema_version() {
    ...
    assert_eq!(
        json["schema_version"], "local-ci.result.v1",
...
fn test_json_all_cached() {
    ...
    let stages = json["stages"].as_array().unwrap();
    let cached_count = stages
        .iter()
        .filter(|s| s["cache_hit"].as_bool().unwrap_or(false))
        .count();
...
fn test_json_compact() {
    ...
    assert!(
        ...
        "compact JSON should not start with space"
    );
}
...
fn test_json_roundtrip() {
    ...
    let json_str = serde_json::to_string(&result).expect("failed to serialize");
    let deserialized: MockPipelineResult =
